...
function discover_tests($start_folder=null, $ignore_onlys=false)
{
    ...
    if (is_array($folders)) {
        foreach ($folders as $folder) {
            ...
            } elseif (is_file($start_folder .'/'. $folder)) {
...
if ($is_cli) {
    ...
    } elseif (array_key_exists('b', $cli_opts) || array_key_exists('bf_only', $cli_opts)) {
        $ignore_folders[] = str_replace('src', 'tests', APP_DIR);
    } elseif (array_key_exists('f', $cli_opts) || array_key_exists('file', $cli_opts)) {
        $only_file = array_key_exists('file', $cli_opts) ? $cli_opts['file'] : $cli_opts['f'];
    } elseif (array_key_exists('d', $cli_opts) || array_key_exists('dir', $cli_opts)) {
...
// Setup similar options via $_GET vars for the GUI version
    ...
    } elseif (isset($_GET['b']) || isset($_GET['bf_only']) || isset($_POST['bf_only'])) {
        $ignore_folders[] = rtrim(str_replace('src', 'tests', APP_DIR), '/');
    } elseif (isset($_GET['f']) || isset($_GET['file'])) {
        $only_file = isset($_GET['f']) ? $_GET['f'] : $_GET['file'];
    } elseif (isset($_GET['d']) || isset($_GET['dir'])) {
