...
class Budget extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('User/dashboard_model'));
    }
    ...
    {
        {
            ...
            if ($this->form_validation->run() === false) {
                Template::set('pageType', $pageType);
                Template::set('pageName', $pageName);
                Template::set_message('Form Validation Failed!', 'error'); 
                Template::redirect($this->uri->uri_string());
            } else {
                // set variables from the form
                $created_by		 			= $this->input->post('user_id');
                $created_by_email			= $this->input->post('user_email');
                $unix_timestamp             = time(); 
                ...
                $designated_date            = $this->input->post('designated_date'); 
                 
                if ($this->budget_model->add_account($created_by, $created_by_date, $unix_timestamp, $month, $day, $year, $time, $status, $account_type, $source_type, $intervals, $name, $amount, $designated_date)) {
    ...
    public function Budget_Account_Overview()
    {
        $pageType = 'Standard';
        $pageName = 'Dashboard';
        ...
        Template::set('pageType', $pageType);
        Template::set('pageName', $pageName);
        Template::set_view('User/Budget/Overview');
        Template::render();
    }
