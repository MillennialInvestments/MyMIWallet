...
class Referral_Program extends Admin_Controller
{
    ...
    public function Apply()
    {
        ...
        if ($this->form_validation->run() === false) {
            $this->load->view('Referral_Program/Apply');
            Template::set('pageType', $pageType);
            Template::set('pageName', $pageName);
            Template::render();
        } else {
            ...
            //~ $cash_app								= $this->input->post('cash_app');
            //~ $venmo									= $this->input->post('venmo');
            $other_payment							= $this->input->post('other_payment');
            
            if ($this->referral_model->apply($user_id, $referrer_code, $active, $signup_date, $user_type, $first_name, $last_name, $email, $phone, $address, $city, $state, $country, $zipcode, $paypal, $other_payment)) {
                if ($this->referral_model->update_user($user_id, $signup_date, $referrer_code, $first_name, $last_name)) {
                    // user creation ok
                    Template::set_message('Your Affiliate Application was Submitted Successfully. Please give us 24-48 Hrs. to review and approved your request.', 'success');
                    redirect('/Profile');
                } else {
                    $email_message_date = array(
                        'user_id'			=> $user_id,
                        ...
                        urn $this->db->insert('bf_users_referral_program', $user);
                    ...
                    $alert = array(
                        ...
                        'to'		=> $email,
                        'subject'	=> $subject,
                        'message'	=> $this->load->view('Referral_Program/Emails/New_Affiliate_Application', $email_message_data, true),
                    );

                    $this->emailer->send($alert);
                    // user creation failed, this should never happen
                    $data->error = 'There was a problem updating your user account. Please try again.';
                    
                    // send error to the view
                    $this->load->view('Referral_Program/Apply', $data);
