...
class MyMIAnalytics
{
    ...
    {    
        ...
        $assetPercentage                    = (($approvedAssets['totalApprovedAssets'] / $target['targetAssets']) * 100) . '%'; 
        $partnerAssetPercentage             = (($approvedPartnerAssets['totalApprovedPartnerAssets'] / $target['targetPartnerAssets']) * 100) . '%'; 
        $transactionPercentage              = (($totalTransOrders['totalTransactions'] / $target['targetTransactions']) * 100) . '%'; 
        $tradesPercentage                   = ($trackedTrades['totalTradesTracked'] / $target['targetTrades']) * 100 . '%'; 
        $partnerPercentage                  = ($activePartners['totalActivePartners'] / $target['targetPartners']) * 100 . '%'; 
        $usersPercentage                    = ($activeUsers['totalActiveUsers'] / $target['targetUsers']) * 100 . '%'; 
        $walletsPercentage                  = ($activeWallets['totalWalletsCreated'] / $target['targetWallets']) * 100 . '%'; 
        // $transAmountPercentage              = ($totalAmounts['totalTransTotals'] / $target['targetTransAmounts']) * 100 . '%'; 
        // $transFeesPercentage                = ($totalAmounts['totalTransFees'] / $target['targetTransFees']) * 100 . '%'; 
    ...
    public function get_total_amount() {
        ...
        foreach($getTotalAmounts->result_array() as $totalAmounts) {
            if ($totalAmounts['fees'] > 0) {
                ...
                $totalTransFeesPlain        = number_format($totalAmounts['amount'], 2); 
    ...
    public function get_pending_support() {
        ...
        $totalPendingSupport                = $getPendingSupport->num_rows(); $getPendingSupport                  = $this->CI->analytical_model->get_pending_support($department);
