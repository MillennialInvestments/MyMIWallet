...
class MyMIAnalytics
{
    ...
    public function get_pending_asset_by_id($appID) {
        $getPendingAssetByID                = $this->CI->analytical_model->get_pending_asset_by_id($appID)->result; 
        foreach ($getPendingAssetByID->result_array() as $pendingAsset) {
            $userID                         = $pendingAsset['user_id']; 
        };

        $getUserInfo                        = $this->CI->analytical_model->get_user_information($userID); 

        $pendingAssetByID                   = array(
            'pendingAsset'                  => $getPendingAssetByID,
