...
class MyMIAnalytics
{
    ...
    public function get_total_amount() {
        ...
        $getTotalPartnerAmounts             = $this->CI->analytical_model->get_total_partner_amounts(); 
        ...
        foreach($getTotalPartnerAssets->result_array() as $partnerAssets) {
            if ($partnerAssets['fees'] > 0) {
                $totalPTransFees             = '<strong>$' . number_format($partnerAssets['fees'],2) . '</strong>';
            } elseif ($partnerAssets['fees'] < 0) {
                $totalTransFees             = '<strong class="statusRed">-$' . number_format($partnerAssets['fees'],2) . '</strong>';
            }
            if ($partnerAssets['amount'] > 0) {
                $totalTransTotals           = '<strong>$' . number_format($partnerAssets['amount'],2) . '</strong>';
            } elseif ($partnerAssets['amount'] < 0) {
                $totalTransTotals           = '<strong class="statusRed">-$' . number_format($partnerAssets['amount'],2) . '</strong>';
            }
            $totalTransFeesPlain            = number_format($partnerAssets['fees'], 2, '.', ''); 
            $totalTransTotalsPlain          = number_format($partnerAssets['amount'], 2, '.', '');
        }
        $totalAmounts                       = array(
