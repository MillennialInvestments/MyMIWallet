...
interface FuncOperation {
    ...
    subTopicOperation: Array<Array<Array<FuncOperator> | FuncOperator>>; //As above, but one for each block. Again, if missing they get repeated
}
 ...
 */
async function trueFalseCancelPrompt(text: string, options: { trueTxt: string; falseTxt: string; cancelTxt: string } = { trueTxt: "Yes", falseTxt: "No", cancelTxt: "Cancel" }): Promise<0 | 1 | 2> {
    const prompt = spawnDiv();
    document.body.append(prompt);
    prompt.agd("promptBox");
    ...
    const result: 0 | 1 | 2 = await new Promise(function (resolve) {
        window.addEventListener("mousedown", function (e) {
            if (e.target != prompt && e.target != trueBtn && e.target != falseBtn) {
                prompt.remove();
                resolve(0);
...
class Table {
     ...
     */
    constructor(parent: HTMLElement, originalChildrenArray: Row2[] = [], tradeWindowRef: TradeWindow, tagObj: refSortingTag) {
        this.parent = parent;
        this.target = "";
        this.tagObj = tagObj;
    ...
    refreshPageController() {
        ...
        if (currentPage != "") {
            const newMax =
                this.sortedMainChildren.length != 0 && Math.floor(this.sortedMainChildren.length / userPrefs.rowsPerPage) - this.sortedMainChildren.length / userPrefs.rowsPerPage == 0
    ...
    pageBackward(numberOfPages: number = 1) {
        ...
        if (
            this.currentPage - 1 - numberOfPages < 0 ||
    ...
    dropdownChildren(children: Array<Row2>) {
        children.forEach((child) => {
            ...
            if (tagIndex > -1) this.tagObj.trades.splice(tagIndex, 1);
        });
        this.refreshPages();
    }
}
...
class TradeWindow {
                            ...
                            if (row.current[gin("30")] != "[]") {
                                ...
                                list.forEach((partialRowPId) => {
                                    ...
                                    if (tagObj.trades.filter((otherRow) => otherRow.current[gin("00p")] == partialRowPId).length == 0) {
                                        //Backward check
                                        if (this.allRowsObj.hasOwnProperty(partialRowPId)) entireList.push(this.allRowsObj[partialRowPId]);
                                    }
                                });
                            }
                            return true;
    ...
    dropTrades(trades: Array<Row2>) {
        ...
        trades.forEach(trade => {
            ...
            if (inObj) delete this.allRowsObj[trade.current[gin("00p")]];
            if (arrIndex > -1) this.allRows.splice(arrIndex, 1);
            if (child && this.allRowsObj.hasOwnProperty(trade.current[gin("29")])) {
                ...
                if (thisIndx > -1) oldList.splice(oldList.indexOf(trade.current[gin("00p")]), 1);
                else console.error("dropTrades$ The parent of this trade doesn't have it anymore in its reference (child then parent)", trade, this.allRowsObj[trade.current[gin("29")]])
...
class Row2 {
    ...
    getLayout(): sectionsLayout {
        ...
        const associatedBlock = sortings[selectedSorting as keyof typeof sortings].blocks[thisRowTargetedValue as keyof typeof userPrefs.sortings.categories.blocks];
            ...
            childList.forEach((pId) => {
                if (theTable.tradeWindowRef.allRowsObj.hasOwnProperty(pId)) {
                    ...
                    if (mainRow != "") {
                        ...
                        if (this.state.dropDown.target != "") this.state.dropDown.target.innerHTML = this.state.dropDown.expanded ? "˄" : "˅";
                    }
                }
            });
        } else {
    ...
    d_saveChanges = async () => {
        try {
            ...
            Object.values(userPrefs.customFields).forEach((customField) => {
                jsUF[customField.name] = this.current[customField.name as keyof Required<TradeObj>];
                //Remove the given field from the dbobject
                delete dbObject[customField.name as keyof Required<TradeObj>];
            });
            ...
            if (tradeWindow == "") throw ({ msg: "Trade window is not defined for this row", obj: this });
            if (this.state.table == "") throw ({ msg: "Table is not defined for this row", obj: this });

            if (isParent && isChild) throw ({ msg: "Trade is both parent and child", obj: this });
            ...
            if (isChild) //Swap and add the new id in the closed lists
            {
                ...
                const childIdCurrentL: string[] = JSON.parse(parent.current[gin("30")]);
                const childIdOriginL: string[] = JSON.parse(parent.origin[gin("30")]);
                ...
                if (childIdCurrentL.indexOf(this.current[gin("00p")]) < 0) {
                    if (childIdCurrentL.indexOf(dbObject[gin("00p")]) < 0) console.error("d_saveChanges$ for some reason the old pseuoid doesn't appear in the parent list. Pushing the new one anyways");
                    else childIdCurrentL.splice(childIdCurrentL.indexOf(dbObject[gin("00p")]), 1);
    ...
    getValue(property: string, target: "current" | "origin" = "current") {
        return this[target as keyof Row2].hasOwnProperty(property) ? { value: this[target][property as keyof TradeObj], has: true } : { value: "", has: false };
    }
                ...
                if (consequential == true || (await trueFalsePrompt("Are you sure you want to permanently delete this row?"))) {
                    ...
                    if (parent) {
                        ...
                        childIdList.forEach(async (childPseudoId) => {
                            ...
                            if (!tradeWindowRef.allRowsObj.hasOwnProperty(childPseudoId)) return;
            ...
            if (this.state.table == "") {
                ...
                Object.values(availableFields).forEach((field) => {
                    ...
                    //This was made for future implementation of fields like the closed perc one
                        ...
                        //Change the values
                        if (!isNaN(parseFloat(this.current[field.name as keyof TradeObj]))) {
                            this.current[field.name as keyof TradeObj] += ((100 - parseFloat(this.current[field.name as keyof TradeObj])) * percentage) / 100;
                            //Add this property to the editing tab
                            this.addEditingOnStdInput({ target: { name: field.name } } as usableEvent);
                            this.changeValue(field.name);
                        }
    ...
    spawnField(directive: string, propInfo: { value: string; has: boolean }) {
         ...
         */
        const buildLister = (targetValue: string, targetInputHolder: fieldHolder, list: listerObj[]) => {
    ...
    renderRow(fresh = true) {
        ...
        if (this.origin[gin("29")] != "-1") {
            ...
            if (this.state.table == "") {
                ...
                if (this.state.container == "") {
                    console.error(
                        "Table has been assigned, but its container is empty (renderRow) (closed-row)",
                        this,
                        this.state.table.tradeWindowRef.allRowsObj[this.origin[gin("29")]].state.container
                    );
                } else {
            ...
            if (this.state.isLegend) {
                ...
                if (this.state.container != "") this.state.container.agd("legendContainer");
            } else {
                mainRow.agd("mainRow");
            }
        }
                ...
                case 1:
                    ...
                    for (let topicPosition = 0; topicPosition < formula.topics.length; topicPosition++) {
                        ...
                        if (formula.underlyingType == "number") {
                            ...
                            const repeatableOperations: Array<FuncOperator> = [...opTopicStreak].filter((operator) => {
...
function createNewRow(
    startingObj: Partial<TradeObj> = new TradeObj({}),
    options: { separator: string; forcedProperties: Partial<TradeObj>; repeat: number } = { separator: "n", forcedProperties: {}, repeat: 1 }
) {
...
if (mainEditPrefsWindow != null) {
    const editPrefsObj = {
         ...
         * @returns 
         */
        async switchPage(newPage: string, directive: -1 | 0 | 1 | 2 = -1) {
            if (this.elements.pages.hasOwnProperty(newPage)) {

                if (this.state.editing.size >= 1) {
                    try {

                        let actualDirective: 0 | 1 | 2 = 0
        ...
        mirroredUserPrefs: structuredClone(userPrefs) as UserPreferences,
        shadowUserPrefs: structuredClone(userPrefs) as UserPreferences,
        /**
            ...
            */
            ...
            const columnsEditor = () => {
                ...
                for (const [blockKey, blockProps] of Object.entries(this.mirroredUserPrefs.sortings[this.mirroredUserPrefs.selectedSorting].blocks)) {
                     ...
                     * @returns 
                     */
                    const createBasicInput = (label: string, type: string) => {
                        const holder = document.createElement("div");
                    ...
                    const spawnEditor = (layoutName: string, target: HTMLDivElement) => {
                        ...
                        const availableFields = { ...availableFieldsGen(this.mirroredUserPrefs) };
                        ...
                        const sectionsRefreshers: { generator: Function, block: HTMLDivElement }[] = [];

                        for (let index = 0; index < layout.length; index++) {
                            const section = layout[index];
                            const fixedIndex = index;
                            const fieldModifiers = new Map() as Map<string, HTMLDivElement>;
                            if (section.size != "0") {
                                //Create + Initialize
                                //Functions
                                //Event Listeners
                                 ...
                                 * @param showHidden 
                                 */
                                const refreshElementEditor = (elementsEditor: HTMLDivElement, showHidden = false) => {
                                ...
                                scrollabelInput.addEventListener("input", () => {
                                    shadowSelectedSorting.blocks[blockKey].layouts[layoutName][index].fixed = (scrollabelInput as HTMLInputElement).checked;
                                    const tag = `sectionFixed_${simpleHash(JSON.stringify(section) + JSON.stringify(index))}`;
                                    if (section.fixed != (scrollabelInput as HTMLInputElement).checked) {
