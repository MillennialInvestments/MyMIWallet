...
class User_model extends BF_Model
{
    ...
    public function get_user_email($cuID)
    {
        $this->db->from('bf_users');
        $this->db->where('id', $cuID);
        $getUserEmail = $this->db->get();
        return $getUserEmail;
    }
    ...
    public function add_investor_profile($id, $email, $username)
    {
        $users = array(
         'id'					=> $id,
         'email'				=> $email,
        ...
        return $this->db->insert('bf_users_investor_profile', $users);
    }

    public function add_social_networks($id, $email, $username)
    {
        $users = array(
         'id'					=> $id,
         'email'				=> $email,
        ...
        return $this->db->insert('bf_users_social_media', $users);
    }

    public function add_default_wallet($id, $active, $beta, $default_wallet, $exchange_wallet, $market_pair, $market, $username, $email, $broker, $wallet_type, $amount, $nickname)
    {
        $users = array(
         'user_id'				=> $id,
         'active'				=> $active,
...
>>>>>>> 76bba32f875dbfd8e00d213db849802fb5378283
    ...
    public function add_referrer($id, $referrer, $referrer_code)
    {
        $user = array(
            'referrer'					=> $referrer,
            'referrer_code'					=> $referrer_code,
    ...
    public function edit_profile($id, $email, $display_name, $username, $phone, $address, $city, $zipcode)
    {
        $user = array(
            'email'					=> $email,
    ...
    public function edit_meta($id, $state, $country)
    {
        $user = array(
            'state'					=> $state,
            'country'				=> $country,
        ...
        $this->db->where('id', $id);
        return $this->db->update('bf_user_meta', $user);
    }

    public function alert_purchased($exchange, $symbol, $user_id, $purchased)
    {
        $user = array(
            'exchange'					=> $exchange,
            'symbol'					=> $symbol,
        ...
        return $this->db->insert('bf_user_alert_purchases', $user);
    }
