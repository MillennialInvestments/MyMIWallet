...
if ($this->agent->is_mobile()) {
    echo '<table class="table table-responsive default" id="userBudgetingHistoricalDatatable">';
} elseif ($this->agent->is_browser()) {
    echo '<table class="table default" id="userBudgetingHistoricalDatatable">';
}
?>
    ...
    <thead>
        ...
        <tr>
            ...
            <th>Due Date</th>
            <th>Account</th>
            <th class="d-sm-none">Source</th>
            <th class="d-sm-none">Wallet</th>
            <th>Amount</th>
            <th>Subtotal</th>
            <th>Actions</th>
        </tr>
    ...
    <tbody>
        ...
        // echo '
        // <tr>
        //     <td class="d-none">' . date("Y-m-1") . '</td>
        //     <td>' . date("F") . ' 1st, ' . date("Y") . '</td>
        ...
        // // $this->db->where('wallet_type !=', 'Banking');
        // // $this->db->where('wallet_type !=', 'Credit');
        ...
        // //     <td class="d-none">' . date("Y-m-1") . '</td>
        // //     <td>' . date("F") . ' 1st, ' . date("Y") . '</td>
        // //     <td><a class="badge badge-sm badge-dim rounded-pill bg-success text-white px-4" href="' . site_url('Trade-Tracker') . '">Investments</a></td>
        // //     <td class="d-sm-none"></td>
        // //     <td class="d-sm-none"></td>
        ...
        // // </tr>
        // // '; 
        $getBudgets                         = $this->budget_model->get_accounts_order_asc_by_date_paid($cuID);
        // $getUserWallets                     = $this->wallet_model->get_all_wallets($cuID); 
        // print_r($getUserWallets->result_array());
        // print_r($getBudgets->result_array()[1]);
        foreach($getBudgets->result_array() as $account) {
            ...
            <tr>
                ...
                <td class="d-sm-none d-md-block">' . date("F jS, Y", strtotime($account['year'] . '-' . $account['month'] . '-' . $account['day'])) . '</td>
                <td class="d-sm-block d-md-none">' . date("m/d/Y", strtotime($account['year'] . '-' . $account['month'] . '-' . $account['day'])) . '</td>
                <td>' . $account['name'] . '</td>
                <td class="d-sm-none">' . $account['source_type'] . '</td>
                <td class="d-sm-none">';    
                        ...
                        foreach ($getUserWallet->result_array() as $userWallets) {
                            if (!empty($userWallets['wallet_type'] === 'Banking')) {
                                if ($getUserWallet->num_rows() > 0) {
                                    echo '
                                        <a href="' . site_url('Wallets/' . $account['source_type'] . '/Details/' . $account['id']) . '/' . $userWallets['id'] . '"><span>' . $userWallets['nickname'] . '</span></a>
                                    ';
                                }
                            }
                        } 
                                ...
                                foreach ($getUserWallets->result_array() as $userWallets) {
                                    if (!empty($userWallets['wallet_type'] === 'Banking')) {
                                        if ($getUserWallets->num_rows() > 0) {
                                            echo '
                                            <li><a href="' . site_url('Wallets/Attach-Account/' . $account['source_type'] . '/' . $account['id']) . '/' . $userWallets['id'] . '"><span>' . $userWallets['nickname'] . '</span></a></li>    
                                            ';
                                        }
                                    }
                                }      
