...
if (!empty($userAccount)) {
    ...
    if (!empty($getUserInfo->result_array())) {
        echo '<script>console.log("Using Direct Connection in Add Wallet");</script>';
...
<div class="modal-body">
    <!-- <div class="row justify-content-center">
        <div class="col-12">
            <div class="card card-bordered pricing">
                <div class="card-body">
                    <h4 class="card-title">Connect <?php //echo $connectModalTitle; ?></h4>
                    <p class="card-description"> Please fill out information below</p>	
                    <a class="btn btn-primary text-white" id="plaid-button">Search Institutions</a>
                </div>
            </div>
        </div>
    </div> -->
...
<script type="text/javascript">
    window.onPlaidLoad = function() {
        var plaidLink = Plaid.create({
            clientName: 'MyMI Wallet',
            env: 'sandbox',
            apiVersion: 'v2',
            key: 'Xtl2psdJQEU61NKKVy2ymmkV29RU98GzrRQyrbFy',
            product: ['auth', 'transactions'],
            onSuccess: function(public_token, metadata) {
            // Send the public_token and account ID to your server
