...
if ($dji_perChange >= 0) {
    $djiScore				= 1;
    $djiScoreCard			= '<span class="statusGreen">+' . $djiScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($dji_perChange <= 0) {
    $djiScore				= -1;
    $djiScoreCard 			= '<span class="statusRed">' . $djiScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($sp_perChange >= 0) {
    $spScore				= 1;
    $spScoreCard			= '<span class="statusGreen">+' . $spScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($sp_perChange <= 0) {
    $spScore				= -1;
    $spScoreCard 			= '<span class="statusRed">' . $spScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($ndx_perChange >= 0) {
    $ndxScore				= 1;
    $ndxScoreCard 			= '<span class="statusGreen">+' . $ndxScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($ndx_perChange <= 0) {
    $ndxScore				= -1;
    $ndxScoreCard 			= '<span class="statusRed">' . $ndxScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($rut_perChange >= 0) {
    $rutScore				= 1;
    $rutScoreCard 			= '<span class="statusGreen">+' . $rutScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($rut_perChange <= 0) {
    $rutScore				= -1;
    $rutScoreCard 			= '<span class="statusRed">' . $rutScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
$usScore 					= $djiScore + $spScore + $ndxScore + $rutScore;
// Calculate INT Equity Score
if ($nik_perChange >= 0) {
    $nikScore				= 1;
    $nikScoreCard			= '<span class="statusGreen">+' . $nikScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($nik_perChange <= 0) {
    $nikScore				= -1;
    $nikScoreCard 			= '<span class="statusRed">' . $nikScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($euro_perChange >= 0) {
    $euroScore				= 1;
    $euroScoreCard 			= '<span class="statusGreen">+' . $euroScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($euro_perChange <= 0) {
    $euroScore				= -1;
    $euroScoreCard 			= '<span class="statusRed">' . $euroScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($dax_perChange >= 0) {
    $daxScore				= 1;
    $daxScoreCard 			= '<span class="statusGreen">+' . $daxScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($dax_perChange <= 0) {
    $daxScore				= -1;
    $daxScoreCard 			= '<span class="statusRed">' . $daxScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
...
if ($vix <= 0) {
    $vixScore				= 1;
    $vixScoreCard 			= '<span class="statusRed">' . $vixScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
} elseif ($vix >= 0) {
    $vixScore				= -1;
    $vixScoreCard 			= '<span class="statusGreen">+' . $vixScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
}
if ($crude >= 0) {
    $crudeScore				= 1;
    $crudeScoreCard 		= '<span class="statusGreen">+' . $crudeScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($crude <= 0) {
    $crudeScore				= -1;
    $crudeScoreCard 		= '<span class="statusRed">' . $crudeScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
if ($hg_perChange >= 0) {
    $hgScore				= 1;
    $hgScoreCard 			= '<span class="statusRed">' . $hgScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
} elseif ($hg_perChange <= 0) {
    $hgScore				= -1;
    $hgScoreCard 			= '<span class="statusGreen">+' . $hgScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
}
if ($audjpy >= 0) {
    $audjpyScore			= 1;
    $audjpyScore 			= '<span class="statusGreen">+' . $audjpyScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($audjpy <= 0) {
    $audjpyScore			= -1;
    $audjpyScoreCard		= '<span class="statusRed">' . $audjpyScore . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
$us_mark					= round(($dji_netChange + $sp_netChange + $ndx_netChange + $rut_netChange)/4, 2);
$usScoreCard 				= $djiScore + $spScore + $ndxScore + $rutScore;
if ($usScoreCard >= 0) {
    $usScoreCard 			= '<span class="statusGreen">+' . $spScore . ' </span> <span class="pl-3" style="color: green;">Bullish</span>';
} elseif ($usScoreCard <= 0) {
    $usScoreCard			= '<span class="statusRed">' . $usScoreCard . ' </span> <span class="pl-3" style="color: red;">Bearish</span>';
}
