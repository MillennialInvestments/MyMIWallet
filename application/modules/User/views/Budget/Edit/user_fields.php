...
<input type="hidden" class="form-control" name="recurring_account_id" id="recurring_account_id" value="<?php echo set_value('recurring_account_id', isset($user) ? $user->recurring_account_id : $accountID); ?>">	
<input type="hidden" class="form-control" name="recurring_account_primary" id="recurring_account_primary" value="<?php echo set_value('recurring_account_primary', isset($user) ? $user->recurring_account_primary : $accountRecurringPrimary); ?>">	

<div class="<?php echo $formGroup; ?> mb-2">    
    <label class="col-6 form-label" for="default-01">Due Date</label>    
    <div class="col-6 form-label" id="designated_date_container">       
       <?php //echo $designatedDateText; ?>	   
        <input type="text" class="<?php echo $formControl; ?> date-picker" name="designated_date" id="designated_date" placeholder="<?php echo $accountDesignatedDate; ?>" value="<?php echo set_value('designated_date', isset($user) ? $user->designated_date : $accountDesignatedDate); ?>">	
    </div>
</div>
<script>
    const designatedDateValue = document.getElementById('#designated_date'); 
    console.log(designatedDateValue);
    // $(function () {
    //     $("#designated_date").datepicker("setDate", <?php echo '"'  . $newAccountDate . '"'; ?>);
    // });
</script>
<div class="<?php echo $formGroup; ?> mb-2">    
	...
	<div class="col-6">
        ...
        <select name="account_type" class="<?php echo $formSelectPicker; ?>" id="account_type" required="required" style="height: 40px; padding: 10px;">
            <?php
	...
	<div class="col-6">
        <select name="is_debt" class="<?php echo $formSelectPicker; ?>" id="is_debt" required="required" style="height: 40px; padding: 10px;">
            <?php
                ...
                foreach ($account_type_values as $value => $display_text) {
                    $selected = ($value == $this->input->post('is_debt')) ? ' selected="selected"' : "";
