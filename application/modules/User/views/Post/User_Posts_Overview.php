...
if ($this->uri->segment(1) === 'Dashboard') {
	$groupID									= 0;
} elseif ($this->uri->segment(1) === 'Stock' OR $this->uri->segment(1) === 'ETF') {
...
foreach ($getPosts->result_array() as $posts) {
	...
	if (! empty($postVideoLink)) {
		...
		<div class="col-12 col-sm col-md-11">
	...
	foreach ($getUserInfo->result_array() as $userInfo) {
		...
		$postLikes									= $getPostLikes->num_rows(); 
		// User Post
		echo'
		<div clas="container-fluid border-dark" style="border-bottom: none;">
			<div class="row pt-4"> 
				...
				<div class="col-10 col-sm-11">
					...
					<br>
					<p class="text-muted mb-0" style="font-weight: 500;">' . $postDate . ' - ' . $postTime . '</p>';
			...
			<div class="row">
				<div class="col-1"></div>
				<div class="col">
					<p class="text-muted mb-0">' . $postLikes . ' Likes</p>
				</div>
			</div>
			...
			foreach ($getLastComment->result_array() as $comment) {
				...
				// Get/Set Post Likes
				$getPostLikes								= $this->investor_model->get_post_likes($commentID);
				$commentLikes								= $getPostLikes->num_rows(); 
				...
				<div class="row pt-5 px-3">
					...
					<div class="col-10 col-sm-8">
						<h5 class="card-title mb-0">
							<span class="float-left"><a href="' . site_url('/Profile/' . $commentUserID) . '">' . $comDisplayName . '</a> <small class="text-muted">' . $comType . ' ' . $cache . '</small></span>
						</h5>
						<br>
				...
				<div class="row">
					<div class="col-1"></div>
					<div class="col">
						<p class="text-muted mb-0">' . $commentLikes . ' Likes</p>
					</div>
				</div>
				<div class="row mt-0">
