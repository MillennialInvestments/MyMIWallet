...
<div class="<?php echo $formGroup; ?>">
	...
	<div class="col-12">    
        <select name="credit_status" class="<?php echo $formSelectPicker; ?>" id="credit_status" onchange="showDiv(this)" required="required" style="height: 40px; padding: 10px;">
            <?php
                $credit_status_values = array(
                    'N/A'                           => 'Select-An-Option',
                    'Active'    		            => 'Active',
                    'Closed'    		            => 'Closed',
                    'Pending'    		            => 'Pending',
                );
                foreach ($credit_status_values as $value => $display_text) {
                    $selected = ($value == $this->input->post('credit_status')) ? ' selected="selected"' : "";
...
<div class="<?php echo $formGroup; ?>">
	...
	<div class="col-12">        
		<input type="text" class="<?php echo $formControl; ?> calculation" name="current_balance" id="current_balance" onChange="calculateExpression(this); calculateBalance(); return false;" placeholder="Ex: 1500.00" value="<?php echo set_value('current_balance', isset($user) ? $user->current_balance : ''); ?>">	
	</div>
</div>
<div class="<?php echo $formGroup; ?>">
	<label class="col-12">Available Balance</label>
	<div class="col-12">        
		<input type="text" class="<?php echo $formControl; ?>" name="available_balance" id="available_balance" onChange="calculateBalance(); return false;"  onInput="calculate(this)"placeholder="Ex: 1500.00" value="<?php echo set_value('available_balance', isset($user) ? $user->available_balance : ''); ?>">	
	</div>
</div>
