...
class Diligence_model extends BF_Model
{
    ...
    */
    ...
    {
        $this->db->from('bf_users_stock_research');
        $getResearch = $this->db->get();
        return $getResearch;
    }
    
    public function get_user_research($cuID)
    {
        $this->db->from('bf_users_stock_research');
        $this->db->where('user_id', $cuID);
        $this->db->order_by('id', 'DESC');
        $this->db->limit(10);
        $getUserResearch = $this->db->get();
        return $getUserResearch;
    }
    
    public function get_user_last_research($cuID)
    {
        $this->db->from('bf_users_stock_research');
        $this->db->where('user_id', $cuID);
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $getUserLastResearch = $this->db->get();
        return $getUserLastResearch;
    }
    
    public function get_user_next_research($cuID)
    {
        $this->db->from('bf_users_stock_research');
        $this->db->where('user_id', $cuID);
        $this->db->order_by('id', 'DESC');
        $this->db->limit(9, 1);
        $getUserNextResearch = $this->db->get();
        return $getUserNextResearch;
    }
    
    public function get_community_research($cuID)
    {
        $this->db->from('bf_users_stock_research');
        $this->db->where('user_id !=', $cuID);
        $this->db->order_by('id', 'DESC');
        $getCommunityResearch = $this->db->get();
        return $getCommunityResearch;
    }
    
    public function get_most_recent_research()
    {
        $this->db->from('bf_users_stock_research');
        $this->db->where('approved', 'Yes');
        $this->db->order_by('id', 'DESC');
        $getMostRecentResearch = $this->db->get();
        return $getMostRecentResearch;
    }
    
    public function get_most_recent_stock_research($symbol)
    {
        $this->db->from('bf_users_stock_research');
